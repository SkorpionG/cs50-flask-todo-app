...
from database import get_db, get_user_tag, get_user_tags
from middleware import login_required, with_context

tags_bp = Blueprint("tags", __name__)


def validate_tag(name: str | None, color: str | None = None) -> list[str]:
    errors: list[str] = []
    ...
    if not name.replace("-", "").replace("_", "").isalnum():
        errors.append("Tag name can only contain letters, numbers, hyphens, and underscores")

    # Validate color if provided
    if color:
        # Check if color is a valid hex color
        if not re.match(r"^#(?:[0-9a-fA-F]{3}){1,2}$", color):
            errors.append("Invalid color format. Please use hex color (e.g., #FF0000)")
...
def create_tag() -> str | Response:
    ...
    try:
        ...
        cursor.execute("SELECT id FROM tags WHERE user_id = ? AND name = ?", (session["user_id"], name))
...
@login_required
def get_user_tags_api() -> Response:
...
@login_required
def search_user_tags_api() -> Response:
    user_id = session["user_id"]
    tag_name = request.args.get("tag")
    db = get_db()
    cursor = db.cursor()

    cursor.execute("SELECT COUNT(*) as count FROM tags WHERE user_id = ? AND name = ?", (user_id, tag_name))
...
@login_required
def edit_tag(tag_id: int) -> str | Response:
    db = get_db()
    cursor = db.cursor()
...
def delete_tag(tag_id: int) -> Response:
    try:
        ...
        if not tag_id_hidden or tag_id != int(tag_id_hidden):
            flash("Error deleting tag", "danger")
        db = get_db()
        cursor = db.cursor()

        cursor.execute("DELETE FROM tags WHERE id = ? AND user_id = ?", (tag_id, session["user_id"]))
        db.commit()
