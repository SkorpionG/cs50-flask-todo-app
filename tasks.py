...
from database import get_db, get_user_tasks, task_priority_options, task_status_options
from middleware import login_required, with_context

tasks_bp = Blueprint("tasks", __name__)


def convert_datetime_format(datetime_string: str | None) -> str | None:
...
def validate_task(
    title: str | None, description: str | None, due_date: str | None, priority: str | None, status: str | None
) -> list[str]:
    errors: list[str] = []
...
def create_task() -> str | Response:
    ...
    try:
        ...
        cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """,
            (session["user_id"], title, description, due_date, priority, status),
        )
...
def edit_task(task_id: int) -> str | Response:
    db = get_db()
    cursor = db.cursor()
...
@login_required
def update_status(task_id: int) -> Response:
    db = get_db()
    cursor = db.cursor()
    try:
        ...
        if not task_id_hidden or task_id != int(task_id_hidden):
...
def completed_tasks() -> str:
    ...
    return render_template(
        "user/completed.html", tasks=user_tasks, completed_columns=completed_columns, page_title="Completed Tasks"
    )
